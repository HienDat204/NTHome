      ...
      <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          {filters.map((item) => (
            <button
              key={item}
              className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm text-slate-700 transition hover:border-secondary hover:text-secondary"
            >
              {item}
            </button>
          ))}
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {properties.map((property) => (
          <Link
            ...
            className="group overflow-hidden rounded-[2rem] bg-white shadow-lg shadow-slate-200/70 transition hover:-translate-y-1 hover:shadow-2xl"
          >
            <div
